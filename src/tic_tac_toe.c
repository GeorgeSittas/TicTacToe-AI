...
int minimax(bool_t max_plays, bool_t *stop, int turn, int *row, int *col) {
      ...
      for (int j = 0; j < SIZE; j++) {
        if (is_board(i, j, EMPTY)) {
          ...
          if (max_evaluation < evaluation) {
            ...
            /* The best move is decided on the first call of minimax, and if */
            /* a winning move is reached for the computer, the search stops */
...
bool_t winning_sequence_exists(char symbol) {
  for (int i = 0; i < SIZE; i++) {
